...
class User:
        ...
        def hash_password(self, password):
            '''Хеширование пароля с алгоритмом SHA-1'''
            return hashlib.sha1(password.encode()).hexdigest()
        def __eq__(self, other):
            '''Переопределение метода сравнения для проверки пользователя по имени пользователя'''
...
class Video:
    ...
    def __eq__(self, other):
        '''Переопределение метода сравнения для проверки видео по заголовку'''
...
class UrTube:
    ...
    def get_videos(self, search_term):
        '''Возврат списка названий видео с поисковым словом (регистр не учитывается)'''
        search_term_lower = search_term.lower()
        return [video.title for video in self.videos if search_term_lower in video.title.lower()]
    def watch_video(self, title):
        '''Воспроизведение видео, если пользователь вошёл в систему и 18+'''
        ...
        for video in self.videos:
            if video.title == title:
                ...
                for second in range(video.time_now + 1, video.duration + 1):
                    ...
                    time.sleep(1) #задержка на секунду
                print("Конец видео")
                video.time_now = 0
                return
